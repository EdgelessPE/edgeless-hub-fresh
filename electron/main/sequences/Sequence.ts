...
class Sequence {
  ...
  async start(): Promise<Res<any>> {
    ...
    for (; stepIndex < this.seq.length; stepIndex++) {
      ...
      log(`Debug:Prepare step ${seqNode.name} at index ${stepIndex} with input params = ${JSON.stringify(inputParams)}`)
      ...
      moduleInstance.listen((type, payload, allowedCommands) => {
        ...
        if (this.current != null && this.current.name != seqNode.name) {
          log(`Debug:Warning:Trying to update a wrong state, current state = ${this.current.name}, seq name = ${seqNode.name}; Received type = ${type}, payload = ${JSON.stringify(payload)}`)
          return
        }
  ...
  async command(cmd: string, payload: any): Promise<Res<any>> {
    ...
    if (!this.current.allowedCommands.includes(cmd)) {
      return new Err(`Error:Command ${cmd} is not allowed at step ${this.current.name}`)
    }

    log(`Info:Response to command ${cmd} with payload : ${JSON.stringify(payload)}`)
  ...
  async retryCurrentStep(): Promise<Res<any>> {
    ...
    if (mcRes.err) {
      log(`Warning:Cancel step ${this.current.name} before retry error : ${mcRes.val}`)
    }
  ...
  getCurrentObservable(observer: Observer<Current | null>): Observable<Current | null> {
