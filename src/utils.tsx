...
function formatSize(bytes: number, decimalPoints = 2) {
  ...
  return new Size(parseFloat(String((bytes / Math.pow(k, size)).toFixed(decimalPoints))), units[size])
}
...
function formatTimestamp(timestamp: number): string {
  ...
  const cur = new Date()
  const sinceDays = Math.round((cur.getTime() - date.getTime()) / (1000 * 3600 * 24))
